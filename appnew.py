...
if not st.session_state['authenticated']:
    ...
    def provide_download_options(df, file_name_base):
        ...
        with c1:
            st.download_button(
                ...
                key=f"xls_{unique_key}"
            )
        with c2:
            st.download_button(
                label="📥 Download CSV",
                data=df.to_csv(index=True if 'GRAND TOTAL' in df.index else False),
    ...
    def generate_all_report(df_data, group_col_input, title):
        ...
        provide_download_options(final, title.replace(' ', '_'))
    ...
    # --- NEW FUNCTION FOR SUB-TOTALS & SEGMENT SPLIT ---
        ...
        if df is not None:
            ...
            if "Dashboard" in tab_map:
                with tab_map["Dashboard"]:
                    ...
                    st.dataframe(df)
                    provide_download_options(df, "Dashboard_Data")
            ...
            if "OEM Pending Analysis" in tab_map:
                with tab_map["OEM Pending Analysis"]:
                    ...
                    if summary_data:
                        ...
                        provide_download_options(summ_df, "Scheme_Wise_Summary")
                        ...
                        c1, c2, c3 = st.columns([1, 1, 3])
                        with c1: st.download_button("📥 Download Excel", data=to_styled_excel(det_df), file_name="Detailed_Scheme_Wise_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                        with c2: st.download_button("📥 Download CSV", data=det_df.to_csv(index=False), file_name="Detailed_Scheme_Wise_Report.csv", mime="text/csv")
            ...
            if "All Report" in tab_map:
                with tab_map["All Report"]:
                    ...
                    # ✅ UPDATED EXCEL LOGIC FOR OPTIONS 1-4
                    if report_type == "1. Consultant & Segment":
                        ...
                        if grp: 
                            ...
                            if "Sales Consultant Name" in grp:
                                ...
                                st.download_button("📥 Download as Excel", data=to_excel(adv_df), file_name="Consultant_Segment_Detailed.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                            else:
                                st.download_button("📥 Download as Excel", data=to_excel(piv, include_index=True), file_name="Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

                    elif report_type == "2. ASM Performance":
                        grp = [c for c in ["ASM", "Segment"] if c in all_rep_df.columns]
                        if grp: 
                            piv_asm = generate_month_wise_pivot(all_rep_df, grp, start_date=ar_start, end_date=ar_end)
                            ...
                            if "ASM" in grp:
                                ...
                                st.download_button("📥 Download as Excel", data=to_excel(adv_df), file_name="ASM_Performance_Detailed.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                            else:
                                st.download_button("📥 Download as Excel", data=to_excel(piv_asm, include_index=True), file_name="ASM_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

                    elif report_type == "3. Model Wise Performance":
                        grp = [c for c in ["Model", "Segment"] if c in all_rep_df.columns]
                        if grp: 
                            piv_mod = generate_month_wise_pivot(all_rep_df, grp, start_date=ar_start, end_date=ar_end)
                            ...
                            if "Model" in grp:
                                ...
                                st.download_button("📥 Download as Excel", data=to_excel(adv_df), file_name="Model_Wise_Detailed.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                            else:
                                st.download_button("📥 Download as Excel", data=to_excel(piv_mod, include_index=True), file_name="Model_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    
                    elif report_type == "4. Consultant Wise Sale":
                        if "Sales Consultant Name" in all_rep_df.columns:
                            piv_cons_only = generate_month_wise_pivot(all_rep_df, ["Sales Consultant Name"], start_date=ar_start, end_date=ar_end)
                            st.dataframe(piv_cons_only.style.format(format_lakhs).format(subset=["Average"], formatter="{:.1f}"))
                            
                            # Use advanced subtotal logic even if segment isn't selected in dropdown, to show split in excel
                            adv_df = generate_subtotal_excel(all_rep_df, "Sales Consultant Name", start_date=ar_start, end_date=ar_end)
                            st.download_button("📥 Download as Excel", data=to_excel(adv_df), file_name="Consultant_Wise_Sale_Detailed.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    ...
                    def create_consolidate(grp_cols):
                         ...
                         if len(grp_cols) > 1: gt_name = tuple(['GRAND TOTAL'] + [''] * (len(grp_cols) - 1))
                         else: gt_name = 'GRAND TOTAL'
                         ...
                         final = pd.merge(grouped.iloc[:-1], pivot_m, left_index=True, right_index=True, how='left')
                         
                         gt_row_combined = pd.concat([grouped.loc[[gt_name]], pivot_m.loc[[gt_name]]], axis=1)
                    ...
                    if report_type == "5. Consultant Consolidate":
                         ...
                         if grp: 
                             cons_df = create_consolidate(grp)
                             st.dataframe(cons_df.style.format(format_lakhs).format(subset=['Fin In-House %', 'MMFSL Share %', 'Ins In-House %'], formatter="{:.1f}"))
                             st.download_button("📥 Download as Excel", data=to_excel(cons_df, include_index=True), file_name="Consultant_Consolidate_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                         ...
                         if grp: 
                             cons_df = create_consolidate(grp)
                             st.dataframe(cons_df.style.format(format_lakhs).format(subset=['Fin In-House %', 'MMFSL Share %', 'Ins In-House %'], formatter="{:.1f}"))
                             st.download_button("📥 Download as Excel", data=to_excel(cons_df, include_index=True), file_name="Consultant_Consolidate_Model_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
